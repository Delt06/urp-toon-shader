...
{
    const half fur_length = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _FurLength);
    const half fur_step = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _FurStep);
...
{
    const half mask = SAMPLE_TEXTURE2D(_FurMask, sampler_FurMask, uv);
    clip(mask - 1);
    const float4 fur_noise_st = UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, _FurNoise_ST);
